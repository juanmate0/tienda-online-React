...
class UsuariosViewSet(viewsets.ModelViewSet):
    ...
    def get(self,request):
        if request.user.username:
            result = self.queryset.filter(username= request.user.username)
            if result:
                return Response({"username": result[0].first_name}, status=status.HTTP_200_OK)
        return Response({"message": "No data sent"}, status=status.HTTP_400_BAD_REQUEST)